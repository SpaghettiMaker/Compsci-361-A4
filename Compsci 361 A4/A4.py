        ...
        for word in datalist:
            if word in supercount.keys():
                ...
                if word in classtype.keys():
                    probability[i] += math.log10(weight*classtype[word]/super_dict[word]) + math.log10(class_freq[i]) - math.log10(super_dict[word])
            else:
                if word in classtype.keys():
                    probability[i] += math.log10(classtype[word]/super_dict[word]) + math.log10(class_freq[i]) - math.log10(super_dict[word])
        i += 1
    return probability



class_freq = [128, 1602, 2144, 216] # fequencey of class a, b, e, v from training data
...
with open('Supercount.json') as file:
    supercount = Counter(json.load(file)) # supercount holds number of times word 'X' appears uniquely across all abstracts

super_dict = Counter() # super_dict holds number of times word 'X' appears across all abstracts
